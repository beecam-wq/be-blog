...
| タグページ | `src/pages/tags/[tag].astro` |
...
| RSS フィード | `src/pages/rss.xml.ts` → `/rss.xml` |
| Vercel デプロイ | `https://be-blog-web.vercel.app` |
| microCMS Webhook | 記事公開・更新時にVercel自動リビルド |
| 記事「五月になった」 | `content/posts/2026-05-01_may-first.md` + microCMS投稿済み |
...
- microCMSのセレクトフィールドは**配列**で返る
  ...
  ```
- microCMSの `tags` フィールドは**文字列**で返ることがある（配列とは限らない）
  → `parseTags()` ヘルパーで正規化（`src/lib/microcms.ts`）
  ```ts
  // "daily golden-week" → ["daily", "golden-week"]
  parseTags(post.tags)
  ```
...
- フッター著者名: `beecam`（本名は使わない）

---

## 次にやること

1. **デザイン改善** — 実際に使いながら気になったところを直す
2. 記事を増やしていく
